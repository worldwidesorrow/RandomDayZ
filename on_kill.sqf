private ["_rockets","_launcher","_type","_skin","_gain","_mission","_ainum","_unit","_player","_humanity","_banditkills","_humankills","_humanitygain","_cash","_randomGain"];
...
if (isPlayer _player) then {
	...
	_randomGain = round (random _gain);
	
	if (ai_humanity_gain) then {
		call {
			if (_unit getVariable ["Hero", false]) exitWith { _player setVariable ["humanity",(_humanity - _randomGain),true]; };
			if (_unit getVariable ["Bandit", false]) exitWith { _player setVariable ["humanity",(_humanity + _randomGain),true]; };					
			if (_unit getVariable ["Special", false]) exitWith { if (_humanity < 0) then { _player setVariable ["humanity",(_humanity - _randomGain),true]; } else { _player setVariable ["humanity",(_humanity + _randomGain),true]; }; };
		};
	};
	
	if (ai_killfeed && ai_humanity_gain) then {
		_aitype = if (_unit getVariable ["Hero", false]) then {"Hero";} else {"Bandit";};
		_humanityReward = if (_aitype == "Hero") then {format["-%1 Humanity",_randomGain];} else {format["+%1 Humanity",_randomGain];};
